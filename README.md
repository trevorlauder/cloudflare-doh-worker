...
> [!IMPORTANT]
> This document may not reflect the latest release. For current documentation, visit the [2.1.0 README](https://github.com/trevorlauder/cloudflare-doh-worker/tree/2.1.0).
...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/trevorlauder/cloudflare-doh-worker/tree/deploy-2.1.0)
...
- Fork this repo on GitHub, then clone your fork and reset `main` to the latest deploy tag:
  ...
  git checkout -B main deploy-2.1.0
...
| `FANOUT_DRAIN_TIMEOUT_MS` | `10000`                                                             | How long to let in-flight provider requests settle in the background after the fan-out deadline                                                         |
...
git fetch upstream
git merge --allow-unrelated-histories deploy-2.1.0  # replace with the new version tag
```
