...
> [!IMPORTANT]
> This document may not reflect the latest release. For current documentation, visit the [2.0.0 README](https://github.com/trevorlauder/cloudflare-doh-worker/tree/2.0.0).
...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/trevorlauder/cloudflare-doh-worker/tree/deploy-2.0.0)
...
- Fork this repo on GitHub, then clone your fork and reset `main` to the latest deploy tag:
  ...
  git checkout -B main deploy-2.0.0
...
| Community block lists ([`blocklist_sources.yaml`](blocklist_sources.yaml)) | Lists are fetched, merged, and bundled as Workers Assets. Small filters are loaded on first request and cached in memory. Filters between 25 and 50 MB are split into 2 shards that both fit in the shard cache. Filters over 50 MB are sharded into ~512 KB pieces, with one shard fetched per request | Large curated lists that are too big to put in config                         |
...
git fetch upstream
git merge --allow-unrelated-histories deploy-2.0.0  # replace with the new version tag
```
