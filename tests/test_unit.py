...
def test_select_winner_rebind_replaced_when_protection_on(monkeypatch):
...
def test_select_winner_rebind_kept_when_protection_off(monkeypatch):
...
def test_select_winner_rebind_no_clean_alternative_kept(monkeypatch):
  monkeypatch.setattr(config, "REBIND_PROTECTION", True)
  rebind = _result(main=True, rebind=True)
  assert _select_winner([rebind]) is rebind


def test_select_winner_prefers_non_rebind_main_over_additional(monkeypatch):
  monkeypatch.setattr(config, "REBIND_PROTECTION", True)
...
def test_resolve_secrets_substitutes():
  ...
  assert (
    _resolve_secrets("prefix-${MY_SECRET}-suffix", env) == "prefix-secret_value-suffix"
  )
...
def test_validate_config_valid(monkeypatch):
  ...
  monkeypatch.setattr(
    config, "BYPASS_PROVIDER", {"host": "dns.example.com", "path": "/dns-query"}
  )
  _validate_config()


def test_validate_config_bypass_missing_host(monkeypatch):
  monkeypatch.setattr(config, "ALLOWED_DOMAINS", ["example.com"])
...
def test_validate_config_bypass_missing_path(monkeypatch):
  monkeypatch.setattr(config, "ALLOWED_DOMAINS", ["example.com"])
...
def test_validate_config_bypass_empty_host(monkeypatch):
  monkeypatch.setattr(config, "ALLOWED_DOMAINS", ["example.com"])
...
def test_negotiate_accept_picks_first_supported():
  assert _negotiate_accept("text/html, application/dns-json") == "application/dns-json"
